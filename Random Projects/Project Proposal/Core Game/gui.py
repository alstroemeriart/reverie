...
class TreeMap:
    ...
    def __init__(self, canvas: tk.Canvas):
        self.canvas = canvas
        self._nodes: list[dict] = []       # {id, type, x, y, state, children}
        self._current_id: int = -1
        self._history: list[str] = []
        self._pending_branches: list[dict] = []
        self._redraw()  # Draw initial state showing "Map loads when run starts"
        # Rebuild and redraw when canvas is resized
    ...
    def _rebuild(self):
        ...
        if canvas_width <= 1:
            # Canvas not yet rendered, use a reasonable default
            canvas_width = 200  # Default when not rendered
        
        # For height, try to get actual rendered height
        canvas_height = self.canvas.winfo_height()
        if canvas_height <= 1:
            canvas_height = 300  # Default fallback
        ...
        # Always create start node
        self._nodes.append({"id": node_id, "type": "start", "x": cx, "y": y,
                             "state": "done", "children": [], "parent": -1})
        current_parent = 0
        node_id += 1

        if self._history:
            # Draw history chain
            for i, ntype in enumerate(self._history):
                y += 52
                node = {"id": node_id, "type": ntype, "x": cx, "y": y,
                        "state": "done" if i < len(self._history) - 1 else "current",
                        "children": [], "parent": current_parent}
                self._nodes[current_parent]["children"].append(node_id)
                self._nodes.append(node)
                current_parent = node_id
                node_id += 1

        # Draw pending branches
        if self._pending_branches:
            branch_count = len(self._pending_branches)
            spacing = min(70, (canvas_width - 40) // max(1, branch_count))
            start_x = cx - (branch_count - 1) * spacing // 2
            branch_y = y + 52
            for i, br in enumerate(self._pending_branches):
                bx = start_x + i * spacing
                bnode = {"id": node_id, "type": br["type"], "x": bx, "y": branch_y,
                         "state": "future", "children": [], "parent": current_parent}
                self._nodes[current_parent]["children"].append(node_id)
                self._nodes.append(bnode)
                node_id += 1
    ...
    def _redraw(self):
        ...
        # Get canvas dimensions - prefer actual rendered size
        canvas_width = c.winfo_width()
        canvas_height = c.winfo_height()
        
        # If not yet rendered, use reasonable defaults
        if canvas_width <= 1:
            canvas_width = 200
        if canvas_height <= 1:
            canvas_height = 300
        ...
        for y in range(0, H, 20):
            c.create_line(0, y, W, y, fill=C["panel3"], width=1)

        if not self._nodes:
            c.create_text(W // 2, H // 2, text="Map loads\nwhen run starts",
                          fill=C["text_dim"], font=("Courier New", 9), justify="center")
            return
        ...
        for node in self._nodes:
            ...
            for child_id in node["children"]:
                ...
                cx2, cy2 = child["x"], child["y"]
                state = child["state"]
                color = C["node_done"] if state == "done" else \
                        C["node_current"] if state == "current" else C["text_muted"]
                dash = () if state != "future" else (4, 4)
                c.create_line(nx, ny + 10, cx2, cy2 - 10,
                              fill=color, width=2, dash=dash, smooth=True)
        ...
        for node in self._nodes:
            ...
            if state == "done":
                outline, fill_c = C["node_done"], C["panel2"]
            elif state == "current":
                outline, fill_c = C["node_current"], "#2d1f12"
            else:
                outline, fill_c = C["text_muted"], C["panel"]

            icon_char, icon_col = self.NODE_ICONS.get(ntype, ("·", C["text_dim"]))

            c.create_oval(nx - r, ny - r, nx + r, ny + r,
                          fill=fill_c, outline=outline, width=2)
            c.create_text(nx, ny, text=icon_char,
                          fill=icon_col if state != "done" else C["text_dim"],
                          font=("Courier New", 9, "bold"))

            # Label beneath node
            label = ntype.upper()[:3]
            c.create_text(nx, ny + r + 8, text=label,
                          fill=C["text_muted"], font=("Courier New", 7))

        # Legend
        legend_items = [("done", C["node_done"], "Visited"),
                        ("current", C["node_current"], "Current"),
                        ("future", C["text_muted"], "Ahead")]
        lx, ly = 6, H - 38
        for state, col, lbl in legend_items:
            c.create_oval(lx, ly + 2, lx + 8, ly + 10, fill=col, outline="")
            c.create_text(lx + 12, ly + 6, text=lbl, anchor="w",
